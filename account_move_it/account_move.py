...
class account_move(models.Model):
	...
	def create(self,vals):
		...
		if t.journal_id.validar_fecha_periodo:
			if t.date != t.fecha_contable:
				raise UserError( ('El diario tiene validacion de fechas de periodo, y este asiento tiene fechas distintas.'))
...
where am.id = %s)""",  (self.name, self.id))
		...
		if self.journal_id.validar_fecha_periodo:
			if self.date != self.fecha_contable:
				raise UserError( ('El diario tiene validacion de fechas de periodo, y este asiento tiene fechas distintas.'))

		for i in self.line_ids:
			i.fecha_contable = self.fecha_contable
		return t
...
class account_move_line(models.Model):
	...
	def create(self,vals):
		...
		t.fecha_contable = t.move_id.fecha_contable
		return t

	@api.one
	def write(self,vals):
		vals['fecha_contable'] = self.move_id.fecha_contable
